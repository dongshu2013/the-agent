...
export async function createOrder(env: Env, userId: string, amount: number) {
    ...
    const result = await db.prepare(
        "INSERT INTO orders (user_id, amount, credits) VALUES (?, ?, ?)"
    ).bind(userId, amount, getCreditFromAmount(amount)).run();
    if (!result.success) {
        throw new Error('Failed to create order');
    }
}

export async function updateOrderSessionId(env: Env, orderId: string, sessionId: string) {
    const db = env.UDB;
    const result = await db.prepare(
        "UPDATE orders SET stripe_session_id = ? WHERE id = ?"
    ).bind(sessionId, orderId).run();
    if (!result.success) {
        throw new Error('Failed to update order');
    }
}

export async function updateOrderStatus(env: Env, orderId: string, status: OrderStatus) {
    const db = env.UDB;
    const result = await db.prepare(
...
export async function finalizeOrder(env: Env, orderId: string) {
    ...
    const orders = await db.prepare(
        "SELECT user_id, credits FROM orders WHERE id = ?"
    ).bind(orderId).all();
