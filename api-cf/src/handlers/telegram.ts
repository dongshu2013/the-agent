...
import { getTelegramDialogs, getTelegramMessages, searchTelegramMessages } from '../db/telegram';
...
export class GetTelegramDialogs extends OpenAPIRoute {
  schema = {
    request: {
      query: z.object({
        ...
        is_free: z.string().optional().transform(val => val === 'true' ? true : val === 'false' ? false : undefined),
  ...
  async handle(c: Context) {
    try {
      ...
      // Get Telegram dialogs
      const result = await getTelegramDialogs(
        env,
        userId,
        limit,
        offset,
...
export class GetTelegramMessages extends OpenAPIRoute {
  schema = {
    request: {
      query: z.object({
        ...
        start_timestamp: z.string().optional().transform(val => val ? parseInt(val, 10) : undefined),
        end_timestamp: z.string().optional().transform(val => val ? parseInt(val, 10) : undefined),
  ...
  async handle(c: Context) {
    try {
      ...
      const startTimestamp = query.start_timestamp ? parseInt(query.start_timestamp, 10) : undefined;
...
export class SearchTelegramMessages extends OpenAPIRoute {
  schema = {
    request: {
      query: z.object({
        ...
        top_k: z.string().optional().transform(val => val ? parseInt(val, 10) : 10),
        message_range: z.string().optional().transform(val => val ? parseInt(val, 10) : 2),
  ...
  async handle(c: Context) {
    try {
      ...
      // Search Telegram messages
      const result = await searchTelegramMessages(
        env,
        userId,
        searchQuery,
        chatId,
