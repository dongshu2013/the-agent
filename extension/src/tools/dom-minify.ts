  ...
  private buildSelectorPaths(node: DomNode, parentPath: string = "") {
    ...
    if (node.id) {
      ...
      const name = node.attributes.name
        ...
        : "";
      currentPath +=
        (currentPath ? " > " : "") + `${node.tagName}${type}${name}`;
    } else {
      const siblings = parentPath ? this.findSiblings(node, parentPath) : [];
      const index = siblings.length;
      currentPath +=
        (currentPath ? " > " : "") + `${node.tagName}:nth-child(${index + 1})`;
    }
...
export function parseHtml(html: string): DomTree {
  ...
  html = html
    ...
    .replace(/\\&nbsp;/g, " ")
    // 处理属性值中的转义反斜杠
    .replace(/(\s+[\w-]+=["'])(.*?)["']/g, (match, prefix, value) => {
  ...
  html = bodyMatch[1]
    ...
    .replace(
      /<div[^>]*data-testid="[^"]*extension[^"]*"[^>]*>[\s\S]*?<\/div>/gi,
      ""
    )
    // 移除追踪相关的div
    ...
    .replace(
      /<div[^>]*data-testid="[^"]*tracking[^"]*"[^>]*>[\s\S]*?<\/div>/gi,
      ""
    )
    ...
    .replace(
      /<div[^>]*data-testid="[^"]*analytics[^"]*"[^>]*>[\s\S]*?<\/div>/gi,
      ""
    );
  ...
  if (isInputOrEditable) {
    ...
    if (node.attributes["style"]) {
      ...
      const filteredStyle = style
        .split(";")
        .filter((style) =>
          importantStyles.some((important) => style.includes(important))
        )
        .join(";");
      if (filteredStyle) {
...
function minifyNode(node: DomNode): string {
  ...
  // 2. 简化节点
  let processedNode = simplifyNode(node);
  if (!processedNode) {
    return "";
  }
  ...
  Object.entries(processedNode.attributes).forEach(([name, value]) => {
    // 移除无用属性
    if (
      ["data-at-shortcutkeys", "data-testid", "class", "style", "id"].includes(
        name
      )
    ) {
      return;
    ...
    if (name.startsWith("aria-")) {
      let cleanValue = compressAttributeValue(value);
      if (cleanValue) {
