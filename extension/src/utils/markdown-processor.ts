...
export function processMarkdown(
  ...
  return (
    content
      ...
      .replace(
        /`([^`]+)`/g,
        `<code style="${mergedOptions.inlineCodeStyle}">$1</code>`
      )
      // 处理粗体
      ...
      .replace(
        /^\s*[-*]\s+(.+)$/gm,
        `<li style="${mergedOptions.listItemStyle}">$1</li>`
      )
      // 处理标题
      .replace(
        /^#\s+(.+)$/gm,
        `<h1 style="${mergedOptions.headingStyles?.h1}">$1</h1>`
      )
      .replace(
        /^##\s+(.+)$/gm,
        `<h2 style="${mergedOptions.headingStyles?.h2}">$1</h2>`
      )
      .replace(
        /^###\s+(.+)$/gm,
        `<h3 style="${mergedOptions.headingStyles?.h3}">$1</h3>`
      )
      // 处理引用
      .replace(
        /^>\s+(.+)$/gm,
