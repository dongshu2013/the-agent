...
import type { TelemetryClient, TelemetryInstance, TelemetryEventData } from './telemetry.types';
...
try {
  ...
  console.error('Failed to check MEM0_TELEMETRY:', error);
}
  ...
  async captureEvent(distinctId: string, eventName: string, properties = {}) {
    ...
    const eventProperties = {
      ...
      $process_person_profile:
        distinctId === 'anonymous' || distinctId === 'anonymous-supabase' ? false : true,
      $lib: 'posthog-node',
    };
