...
export class RedisDB implements VectorStore {
  ...
  private async createIndex(): Promise<void> {
    try {
      ...
      // Create the index
      await this.client.ft.create(this.indexName, schema as unknown as RediSearchSchema, {
        ON: 'HASH',
        PREFIX: this.indexPrefix + ':',
        STOPWORDS: [],
      });
  ...
  async initialize(): Promise<void> {
    try {
      ...
      const modulesResponse = (await this.client.moduleList()) as unknown as unknown[];
  ...
  async insert(
    ...
    const data = vectors.map((vector, idx) => {
      ...
      const entry: Record<string, unknown> = {
        ...
        hash: payload.hash,
        memory: payload.data,
        created_at: new Date(payload.created_at as string).getTime(),
      ...
      await Promise.all(
        data.map(entry =>
          this.client.hSet(`${this.indexPrefix}:${entry.memory_id}`, {
            ...entry,
            embedding: Buffer.from(entry.embedding as ArrayBuffer),
          })
        )
      );
    } catch (error) {
  ...
  async update(
    ...
    const entry: Record<string, unknown> = {
      ...
      created_at: new Date(snakePayload.created_at as string).getTime(),
      updated_at: new Date(snakePayload.updated_at as string).getTime(),
    ...
    // Add metadata excluding specific keys
    entry.metadata = JSON.stringify(
      Object.fromEntries(Object.entries(snakePayload).filter(([key]) => !EXCLUDED_KEYS.has(key)))
    );

    const safeEntry: Record<string, string | Buffer | number> = {};
    for (const [k, v] of Object.entries(entry)) {
      if (typeof v === 'string' || typeof v === 'number' || Buffer.isBuffer(v)) {
        safeEntry[k] = v;
      } else if (v instanceof ArrayBuffer) {
        safeEntry[k] = Buffer.from(v);
      } else {
        safeEntry[k] = JSON.stringify(v);
      }
    }

    try {
      await this.client.hSet(`${this.indexPrefix}:${vectorId}`, safeEntry);
    } catch (error) {
