...
export class PGVector implements VectorStore {
  ...
  async getUserId(): Promise<string> {
    const result = await this.client.query('SELECT user_id FROM memory_migrations LIMIT 1');
    ...
    await this.client.query('INSERT INTO memory_migrations (user_id) VALUES ($1)', [randomUserId]);
    return randomUserId;
  }

  async setUserId(userId: string): Promise<void> {
    await this.client.query('DELETE FROM memory_migrations');
    await this.client.query('INSERT INTO memory_migrations (user_id) VALUES ($1)', [userId]);
  }
}
