...
import { DELETE_MEMORY_TOOL_GRAPH, EXTRACT_ENTITIES_TOOL, RELATIONS_TOOL } from '../graphs/tools';
...
export class MemoryGraph {
  ...
  constructor(config: MemoryConfig) {
    ...
    this.structuredLlm = LLMFactory.create('openai_structured', this.config.llm.config);
    this.threshold = 0.7;
  }
  ...
  private async _searchSourceNode(sourceEmbedding: number[], userId: string, threshold = 0.9) {
    const session = this.graph.session();
    try {
      const cypher = `
