...
import { getFactRetrievalMessages, getUpdateMemoryMessages, removeCodeBlocks } from '../prompts';
...
export class Memory {
  ...
  private async _initializeTelemetry() {
    try {
      ...
      console.error('Failed to capture init event:', error);
    }
  }
  ...
  private async _getTelemetryId() {
    try {
      ...
      console.error('Failed to get telemetry ID:', error);
      this.telemetryId = 'anonymous';
      return this.telemetryId;
    }
  }
  ...
  private async addToVectorStore(
    ...
    if (!infer) {
      ...
      for (const message of messages) {
        ...
        const memoryId = await this.createMemory(message.content as string, {}, metadata);
    ...
    try {
      ...
      console.error('Failed to parse facts from LLM response:', cleanResponse, e);
      facts = [];
    }
    ...
    try {
      ...
      console.error('Failed to parse memory actions from LLM response:', cleanUpdateResponse, e);
      memoryActions = [];
    }
